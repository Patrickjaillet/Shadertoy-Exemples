// ==== Image (image) ====
// SSD : https://patrickjaillet.github.io/sandefjord-software
// **************************************************************************************
/* GOLFED VERSION
#define R iResolution
#define M(a) mat2(cos(a),-sin(a),sin(a),cos(a))
void mainImage(out vec4 e,vec2 l){
vec2 m=(l+l-R.xy)/R.y;
float c=iTime,f=0.,h,j,d,i=0.,q;
mat2 n=M(c*.2),o=M(c*.15+.3),p=M(.7854);
for(e=vec4(.1);i++<32.;){
vec3 b=f*vec3(m,1);
b.xz*=n;
b.yz*=o;
...
++b;
h=j=9.;
for(int k=0;k<7;k++)b=mod(b-1.,2.)-1.,b.yz*=p,h=min(h,length(b)),j/=d=dot(b,b)*.6,b/=d;
f+=d=1./j;
q=fract(h+f*.08+c*.04);
e.rgb+=.014/exp(d*1200.+f*.15)*(.78+1.82*clamp(abs(fract(q+vec3(3,2,1)/3.)*20.-3.)-1.,.2,1.));
}
vec3 g=e.rgb;
e=vec4(clamp((g*(2.51*g+.03))/(g*(1.68*g+.59)+.14),0.,1.),1.);
}
*/

// I’ve written what the numbers are for above each line, making it easy to modify whatever you want.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float rotInnerVal = 0.70710678;
    mat2 rotInner = mat2(rotInnerVal, -rotInnerVal, rotInnerVal, rotInnerVal);
