...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    float bass = texture(iChannel1, vec2(0.05, 0.25)).r;
    float kick = pow(bass, 2.5);

    vec2 cc = fragCoord / iResolution.xy - 0.5;
    vec2 distortedUV = 0.5 + cc * (1.0 + dot(cc, cc) * (0.12 + kick * 0.05));
    ...
    float glitch = step(0.95, hash11(iTime * 10.0)) * kick * 0.05;
    vec2 uvR = quantUV + vec2(glitch + 0.005, 0.0);
    vec2 uvG = quantUV;
    vec2 uvB = quantUV - vec2(glitch + 0.005, 0.0);
    ...
    for(float i=-2.0; i<=2.0; i+=1.0) {
        for(float j=-2.0; j<=2.0; j+=1.0) {
            vec2 bUv = quantUV + vec2(i, j) * 0.004;
            bloom += texture(iChannel0, bUv).rgb;
            weightSum += 1.0;
        }
    }
    col += (bloom / weightSum) * (0.4 + kick * 0.5);
    ...
    float scanline = sin(distortedUV.y * iResolution.y * 1.5 + iTime * 5.0);
...
float printLine1(vec2 uv) {
    int msg[35] = int[](
        ...
        35, 48, 48, 48, 48, 48, 48, 48, 52, 46, 48, 48, 48, 48, 48, 48, 48, 53, 32
    );
...
vec3 renderS19(vec2 fc, vec2 res, float time) {
    vec2 uv = fc / res;
    ...
    float scanline = sin(uv.y * res.y * 1.5) * 0.1;
    col -= scanline;
    return col;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float mid  = texture(iChannel0, vec2(0.40, 0.25)).r;
    float high = texture(iChannel0, vec2(0.80, 0.25)).r;
    float kick = pow(bass, 2.5);
    ...
    if (iTime < 45.0) {
        ...
        finalCol = renderAmigaScene(currentCoord, iResolution.xy, iTime, bass, mid, high, kick);
        if (alpha > 0.0) {
        ...
        float hardKick = smoothstep(0.7, 1.0, kick);
        float hatStrobe = smoothstep(0.8, 1.0, high);
        
        int id1 = int(hash11(session) * 20.0) % 20;
        int id2 = int(hash11(session + 1.0) * 20.0) % 20;
        
        vec2 fc1 = currentCoord;
        fc1 = (fc1 - iResolution.xy * 0.5) * (1.0 - hardKick * 0.15) + iResolution.xy * 0.5;

        if (hardKick > 0.5 && hash11(mTime * 12.0) > 0.7) {
            fc1.x += (hash11(mTime * 22.0) - 0.5) * 80.0;
            fc1.y += (hash11(mTime * 33.0) - 0.5) * 50.0;
        }

        if (bass > 0.8 && hash11(session * 2.4) > 0.5) {
        ...
        finalCol = getMegademoScene(id1, fc1, iResolution.xy, mTime + hardKick * 0.2, bass, mid, high, kick);

        float tFX = smoothstep(0.85, 1.0, sProg);
        if (tFX > 0.0) {
            vec2 fc2 = currentCoord;
            float fxType = floor(mod(hash11(session * 3.3) * 4.0, 4.0));
            if (fxType == 1.0) fc2.x += sin(fc2.y * 0.1 + mTime * 20.0) * 100.0 * tFX; 
            else if (fxType == 2.0) fc2 = floor(fc2 / (1.0 + tFX * 32.0)) * (1.0 + tFX * 32.0);
            
            vec3 col2 = getMegademoScene(id2, fc2, iResolution.xy, mTime, bass, mid, high, kick);
            if (fxType == 0.0) finalCol = mix(finalCol, col2, tFX);
            else if (fxType == 1.0) finalCol = mix(finalCol, col2, step(0.5, hash11(fc1.y + mTime))); 
            else finalCol += col2 * tFX * hardKick; 
        }

        float showObj = step(0.25, hash11(session * 4.7));
        if (showObj > 0.5) {
            float objIn = smoothstep(0.0, 0.12, sProg) * (1.0 - smoothstep(0.88, 1.0, sProg));
            vec2 uvVector = (currentCoord - 0.5 * iResolution.xy) / iResolution.y;
            if (sProg > 0.88 && hash11(mTime * 15.0) > 0.4) {
                uvVector.x += (hash11(mTime * 30.0) - 0.5) * 0.25;
            }

            int wireObjType = int(mod(floor(hash11(session * 9.1) * 7.0), 7.0));
            float morphVal = sin(mTime * 2.0) * 0.5 + 0.5;
            float wireDist = drawAmigaWireframeObject(wireObjType, uvVector, mTime, bass, high, kick, morphVal, session);
            
            float lineThick = 0.003 + hardKick * 0.004;
            float lineGlow = exp(-wireDist * (150.0 - hardKick * 50.0));
            ...
            vec3 wireCol = (wirePal * lineGlow * (2.2 + kick * 1.5)) + (vec3(1.0) * lineCore);
            
            float isSolidPhase = step(0.6, hash11(session * 8.1));
            if (isSolidPhase > 0.5 && mid > 0.35) {
                vec3 roS = vec3(uvVector * 3.0, -3.0);
                vec3 rdS = vec3(0.0, 0.0, 1.0);
                rdS.xz *= rot(mTime * 1.5); rdS.yz *= rot(mTime * 0.9);
                float dS = getObjGenericSDF(wireObjType, roS, morphVal, bass, high);
                if (dS < 0.1) {
        ...
        if (hatStrobe > 0.5 && mod(mTime * 15.0, 1.0) > 0.5) {
            finalCol = 1.0 - finalCol; 
            finalCol.rg = finalCol.gr; 
        }

        finalCol += vec3(0.8, 0.9, 1.0) * pow(hardKick, 4.0);
        finalCol.r += smoothstep(0.6, 1.0, bass) * 0.3 * length(currentCoord / iResolution.xy - 0.5);
    }
