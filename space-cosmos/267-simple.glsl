...
vec3 col2 = vec3(0.0, 0.3, 0.2);
vec3 col3 = vec3(0.5, 0.4, 0.1);

col = mix(col1, col2, smoke);
col = mix(col, col3, length(q) * 0.3);

col = pow(col, vec3(1.2));
col *= 1.5;

float stars = pow(Hash21(floor(uv * 40.0)), 25.0);
col += stars * (sin(time * 2.0 + stars * 100.0) * 0.5 + 0.5) * col2;
