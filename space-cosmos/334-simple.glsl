...
void mainImage(out vec4 O, vec2 U) {
    vec4 acc = vec4(0.0);
    for(float m = 0.0; m < 5.0; m++) {
        float t_mb = iTime - (m - 2.0) * 0.016;
        vec2 V = (U+U - iResolution.xy) / min(iResolution.x, iResolution.y);
        float t_cycle = mod(t_mb, 11.0),
              T = t_cycle > 5.5 ? 11.0 - t_cycle : t_cycle,
        ...
        for(int k = 0; k < 3; k++) {
            ...
            if(h > 0.8) {
                float d = length(g);
                float br = smoothstep(0.1, 0.0, d) * (sin(t_mb * (h * 5.0 + 2.0) + h * 6.28) * 0.5 + 0.5);
                Sbg += vec3(0.8, 0.9, 1.0) * br * h * 0.3;
            }
        }
        vec2 U_main = (U+U - iResolution.xy) / min(iResolution.x, iResolution.y);
        ...
        vec3 CP = vec3(0);
        CP.xz = R * CP.xz;
        o += vec3(3, 1.5, 0.6) * exp(-length(U_main - CP.xy / (CP.z + 2.5)) * 4.0) * max(0.0, 1.0 - t * 2.5);
        acc += vec4(o + Sbg, 1.0);
    }
    O = acc / 3.0;
}
